...
* @Last Modified time: 2017-12-15 01:13:36
*/
...
const plaintextPasswordBeda = global.plaintextPasswordBeda;

/* benchmark */
suite
	// add blowfish test 
	.add('Blowfish#test', function(defer) {
	    bcrypt.compare(plaintextPasswordBeda, bcryptHash, function(err, res) {
	        if(res) {
	...
	.add('MD5#test', function(defer) {
		...
		if (hash == md5(plaintextPasswordBeda)) {
			defer.resolve('right password')
		}else{
			defer.resolve('wrong password')
		}
